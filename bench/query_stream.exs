...
Task.async_stream(records_l, fn i ->

  s_f = :erlang.monotonic_time(:microsecond)
  #:erlcass.async_execute(:testing_query, [key])
  {:ok, _Tag} = Session.async_execute(session, ps, [{:text, key}])
  #IO.puts(s_e)
  receive do
    {{:execute, _}, t} ->
      #IO.inspect(t)
  ...
  s_e = :erlang.monotonic_time(:microsecond) - s_f
  #IO.puts(s_e)
end, timeout: 60_000, max_concurrency: concurrency, ordered: false) |> Enum.to_list()
total = :erlang.monotonic_time(:millisecond) - start
IO.puts("Took #{total}ms to execute #{records} records (#{records / (total / 1000)} r/s)")
