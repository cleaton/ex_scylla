defmodule ExScylla.Examples.BatchTest do
  ...
  test "unlogged batch", %{session: session} do
    ...
    {:ok, _} = Session.query(session, "CREATE TABLE unlogged_batch (a int PRIMARY KEY, b text);", [])

    batch = Batch.new(:unlogged)
  ...
  test "prepared batch", %{session: session} do
    # Unique table for this test
    {:ok, _} = Session.query(session, "CREATE TABLE prepared_batch (a int PRIMARY KEY, b text);", [])
