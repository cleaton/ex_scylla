defmodule ExScylla.Examples.StreamTest do
  ...
  setup_all do
    ...
    {:ok, _} = Session.query(session, "CREATE TABLE IF NOT EXISTS stream_table (a int PRIMARY KEY, b text);", [])
    
    # Insert data for streaming tests
    for i <- 1..20 do
      {:ok, _} = Session.query(session, "INSERT INTO stream_table (a, b) VALUES (?, ?)", [int: i, text: "val#{i}"])
    end
  ...
  test "Session.query_stream with page size", %{session: session} do
    ...
    # Query using stream
    rows = Session.query_stream(session, q, [])
           |> Enum.to_list()
  ...
  test "Session.execute_stream", %{session: session} do
    ...
    rows = Session.execute_stream(session, ps, [])
           |> Enum.to_list()
